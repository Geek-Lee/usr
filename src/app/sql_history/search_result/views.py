...
def fofeasy_home(request):
    ...
    # posts1, posts2, posts3, posts5, sum_user
    now = datetime.now()
    before_month_now = now - timedelta(days=30)
    date = now.strftime('%Y-%m-%d')  # 日期
    before_month_date = before_month_now.strftime('%Y-%m-%d')


    sql = "SELECT *, Count(user_id) as visit_times FROM easy_log GROUP BY user_id"
    df = pd.read_sql(sql, engine_ht).sort_values(by='visit_times', ascending=False, inplace=False)
    posts5 = df.to_dict(orient="records")  # 表格数据
    sum_user = df['user_id'].count()

    sql_ip_table = "SELECT DATE_FORMAT(local_time, '%%Y-%%m-%%d') AS date, Count(DISTINCT user_id) AS sum_ip FROM easy_log WHERE (local_time >= '{}'  AND local_time <= '{}') AND user_id is not NULL GROUP BY DATE_FORMAT(local_time, '%%Y-%%m-%%d')".format(before_month_date, date)
    df_ip = pd.read_sql(sql_ip_table, engine_ht)
    df_ip["sum_ip"] = df_ip["sum_ip"].apply(lambda x: float(x))
    # df.to_csv(r'C:\Users\K\Desktop\usr')
    posts1 = df_ip.to_dict(orient="records")
    posts1 = json.dumps(posts1)

    sql_api_table = "SELECT DATE_FORMAT(local_time, '%%Y-%%m-%%d') AS date, Count(request_url) AS sum_api FROM easy_log WHERE (local_time >= '{}'  AND local_time <= '{}') GROUP BY DATE_FORMAT(local_time, '%%Y-%%m-%%d')".format(before_month_date, date)
    df_api = pd.read_sql(sql_api_table, engine_ht)
    posts2 = json.dumps(df_api.to_dict(orient="records"))


    #sql_api_piechart = "SELECT DISTINCT request_url AS user_action, Count(request_url) AS sum_api FROM easy_log WHERE (local_time >= '{}' AND local_time <= '{}')AND user_action is not '{}' GROUP BY request_url".format(before_month_date, date, index_address)
    sql_api_piechart = "SELECT DISTINCT request_url AS user_action, Count(request_url) AS sum_api FROM easy_log WHERE (local_time >= '{}' AND local_time <= '{}') GROUP BY request_url".format(
        before_month_date, date)
    df_api_piechart = pd.read_sql(sql_api_piechart, engine_ht)
    df_api_piechart = df_api_piechart[df_api_piechart['user_action'].str.contains(r'/api/')]
    df_api_piechart = df_api_piechart.sort_values(by="sum_api", ascending=False, inplace=False).head(10)
    posts3 = json.dumps(df_api_piechart.to_dict(orient="records"))


    return render_to_response("fofeasy_home.html",
                              {'posts': posts, 'post1': posts1, 'post2': posts2, 'post3': posts3, 'sum_user': sum_user, 'post4': post4, 'post5': posts5, 'date': date})


@login_required
def fofeasy_home_userid(request, user_id):
    pass
